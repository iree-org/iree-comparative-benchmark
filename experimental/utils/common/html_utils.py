...
def format_latency_comparison(iree_latency, baseline_latency):
  ...
  latency = (faster_label.format(speedup)
             if speedup >= 1.0 else slower_label.format(slowdown))
  return latency
...
def format_memory_comparison(iree_memory, baseline_memory):
  ...
  memory = (smaller_label.format(smaller)
            if smaller >= 1.0 else larger_label.format(larger))
  return memory
