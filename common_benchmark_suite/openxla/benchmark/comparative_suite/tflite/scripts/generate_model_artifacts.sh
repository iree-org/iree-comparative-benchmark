...
#
# Runs `generate_model_artifacts.py` on all registered TFLite models and saves
# artifacts into the directory
# `${OUTPUT_DIR}/tflite_models_<timestamp>`.
#
# Once complete. please upload the output directory to
# `gs://iree-model-artifacts/tflite`, preserving directory name.
#
# Usage:
...
#   GCS_UPLOAD_DIR=gs://iree-model-artifacts/tflite
#   AUTO_UPLOAD=1
#
...
DIR_NAME="tflite_models_$(date +'%s')"
...
declare -a args=(
  -o "${VERSION_DIR}"
  --iree_import_tool="$(which iree-import-tflite)"
